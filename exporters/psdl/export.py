...
try:
    if len(sys.argv) != 15:
        raise Exception("Wrong number of arguments, must be 14 (modules path, input json file, output psdl file, and 11 flags), got " + str(len(sys.argv) - 1))
    ...
    exp = BINExporter(data, int(sys.argv[14]) > 0)
